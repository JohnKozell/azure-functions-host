...
namespace Microsoft.Azure.WebJobs.Script.Workers.Rpc
{
    internal class RpcWorkerChannel : IRpcWorkerChannel, IDisposable
    {
        ...
        internal async Task SendInvocationRequest(ScriptInvocationContext context)
        {
            try
            {
                ...
                else
                {
                    ...
                    var excludeHttpTriggerMetadata = IsHttpTriggerMetadataPopulatedByWorker();
                    var invocationRequest = await context.ToRpcInvocationRequest(excludeHttpTriggerMetadata, _workerChannelLogger, _workerCapabilities);
        ...
        private bool IsHttpTriggerMetadataPopulatedByWorker()
        {
